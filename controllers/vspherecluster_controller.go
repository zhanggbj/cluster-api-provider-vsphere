...
// manager.
func AddClusterControllerToManager(ctx *capvcontext.ControllerManagerContext, mgr manager.Manager, clusterControlledType client.Object, options controller.Options) error {
...
func clusterToInfrastructureMapFunc(managerContext *capvcontext.ControllerManagerContext) handler.MapFunc {
	gvk := infrav1.GroupVersion.WithKind(reflect.TypeOf(&infrav1.VSphereCluster{}).Elem().Name())
	return clusterutilv1.ClusterToInfrastructureMapFunc(context.Background(), gvk, managerContext.Client, &infrav1.VSphereCluster{})
}
