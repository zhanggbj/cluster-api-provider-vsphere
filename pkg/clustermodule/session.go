...
func fetchSessionForObject(ctx *capvcontext.ClusterContext, template *infrav1.VSphereMachineTemplate) (*session.Session, error) {
...
func newParams(ctx capvcontext.ClusterContext) *session.Params {
...
func fetchSession(ctx *capvcontext.ClusterContext, params *session.Params) (*session.Session, error) {
	if ctx.VSphereCluster.Spec.IdentityRef != nil {
		creds, err := identity.GetCredentials(context.Background(), ctx.Client, ctx.VSphereCluster, ctx.Namespace)
		if err != nil {
			return nil, err
		}
...
func fetchMachineTemplate(ctx *capvcontext.ClusterContext, input Wrapper, templateName string) (*infrav1.VSphereMachineTemplate, error) {
